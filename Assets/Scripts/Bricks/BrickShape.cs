...
public class BrickShape : MonoBehaviour
{
    ...
    public void UpdateShape () {
        Vector3 size = Helper.CorrectBHScale(brick.Scale, brick.Rotation);
        size = size.SwapYZ().NotQuiteZeroButClose();

        if (brick.Shape == Map.Brick.ShapeType.cube) {
            // 0 = brick
            elements[0].localScale = size;
        } else if (brick.Shape == Map.Brick.ShapeType.slope) {
        ...
        } else if (brick.Shape == Map.Brick.ShapeType.wedge) {
            // 0 = main, 1 = side
        ...
        } else if (brick.Shape == Map.Brick.ShapeType.spawnpoint) {
            // 0 = spawnpoint
            elements[0].localScale = size;
        } else if (brick.Shape == Map.Brick.ShapeType.arch) {
        ...
        } else if (brick.Shape == Map.Brick.ShapeType.dome) {
            // 0 = dome
            elements[0].localScale = size;
        } else if (brick.Shape == Map.Brick.ShapeType.bars) {
        ...
        } else if (brick.Shape == Map.Brick.ShapeType.flag) {
            // 0 = flag
            elements[0].localScale = size;
        } else if (brick.Shape == Map.Brick.ShapeType.pole) {
            // 0 = base, 1 = mid, 2 = tip
            elements[0].localPosition = new Vector3(0, -size.y / 2, 0);
            elements[1].localScale = new Vector3(1, size.y - 0.6f, 1);
            elements[2].localPosition = new Vector3(0, 0.5f * size.y - 0.3f, 0);
        } else if (brick.Shape == Map.Brick.ShapeType.cylinder) {
            // 0 = cyl, 1 = base
            ...
            elements[1].localPosition = new Vector3(0, -size.y / 2, 0);
        } else if (brick.Shape == Map.Brick.ShapeType.round_slope) {
            // 0 = slope
            elements[0].localScale = size;
        } else if (brick.Shape == Map.Brick.ShapeType.vent) {
            // 0 = main
            int segmentCountX = Mathf.CeilToInt(size.x);
            float firstPosX = -segmentCountX / 2f + 0.5f; // ebic
    ...
    public void SetAssetGameobject (Mesh mesh, Texture2D texture) {
        ...
        if (mr.material == null) {
            mr.material = new Material(Shader.Find("Brick/Normal"));
        }
