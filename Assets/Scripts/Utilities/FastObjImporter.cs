...
public sealed class FastObjImporter
{
    ...
    public Mesh ImportFile(string filePath)
    {
        ...
        for (int i = 0; i < faceData.Count; i++)
        {
            ...
            if (faceData[i].z >= 1)
                if (hasNormals) newNormals[i] = normals[faceData[i].z - 1];
        }
    ...
    private float ParseFloat(StringBuilder value)
    {
        ...
        return result;
        */
        return Convert.ToSingle(value.ToString()); // unoptimized code go brrr
        // you know i had to do it to em because this speedy function doesnt support scientific notation
    }
